...
DLJES has benefited from contributions from Marek Stastna, Kevin Lamb, Chris Subich, Derek Steinmoeller and Jorge Magalh\~aes.
...
\begin{itemize}
 ...
 \item $\bar{\rho}(z)$ and $\deriv{}{z} \bar{\rho}(z)$
       ...
       \begin{itemize}
        ...
        \item If using non-dimensionalised density, DJLES will assume \verb+rho0=1+.
       \end{itemize}
...
This case is intended as a ``real world'' case where we reproduce the wave shown in Figure 11 of \cite{PinedaEtAl2015}.
The text file contains two columns, depth (m) and density (kg/m$^3$), and is an approximate reproduction (courtesy Jorge Magalh\~aes) of the smoothed curve of Fig 9a.
We use the full density here, so we set the reference density \verb+rho0+ as the maximum value.
The density and first derivatives are constructed as in the synthetic data case.
Amplitude is raised in a few steps, and then we apply several steps of resolution refinement with epsilon reductions.
The final wave is solved to a relative residual of order 2e-8.
