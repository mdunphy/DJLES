...
switch gridtype
    case 'interior'
        ...
        [SZ,SX] = size(f);
        fx = real(ifft(bsxfun(@times,1i*ks(:).',fft(cat(2,f,s*compat_flip(f,2)),[],2)),[],2));
        fx = fx(1:SZ,1:SX);
        fz = real(ifft(bsxfun(@times,1i*ms(:)  ,fft(cat(1,f,s*compat_flip(f,1)),[],1)),[],1));
        fz = fz(1:SZ,1:SX);
    case 'endpoint'
        % Cell edges grid or 'endpoint' grid, which is one larger in
        % each dimension than the interior grid
        [SZ,SX] = size(f);
        fx = real(ifft(bsxfun(@times,1i*ks(:).',fft(cat(2,f,s*compat_flip(f(:,2:end-1),2)),[],2)),[],2));
        fx = fx(1:SZ,1:SX);
        fz = real(ifft(bsxfun(@times,1i*ms(:)  ,fft(cat(1,f,s*compat_flip(f(2:end-1,:),1)),[],1)),[],1));
        fz = fz(1:SZ,1:SX);
    otherwise
